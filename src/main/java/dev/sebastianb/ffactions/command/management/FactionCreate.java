    ...
    private static int createFaction(CommandContext<ServerCommandSource> commandContext) {
        ...
        if (FactionManagement.isInFaction(player) && !FactionManagement.isFactionOwner(player)) {
            SebaUtils.ChatUtils.saySimpleMessage(commandContext,
                    Text.literal("You are already in a faction! To create a new faction, leave your current faction.")); // TODO: Replace this with a translatable
            return 0; // 0 for FAIL
        }
        ...
        SebaUtils.ChatUtils.saySimpleMessage(commandContext,
                Text.literal("Successfully created faction \"" + factionName + "\" with the tag of \"" + factionTag + "\"")); // TODO: Replace with translatable
